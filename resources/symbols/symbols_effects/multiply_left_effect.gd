...
func apply_effect(my_index: int, ctx: BattleContext) -> void:
	if my_index > 0 and ctx.board[my_index - 1] != null:
		...
		var my_power = base_multiplier * ctx.buckets[my_index]["multiplier_power"]
		
		target_bucket["multiplier_power"] *= my_power
...
func get_contextual_label(my_index: int, ctx: BattleContext) -> String:
	var power = base_multiplier * ctx.buckets[my_index]["multiplier_power"]
	...
	if ctx.buckets[my_index]["multiplier_power"] > 1:
